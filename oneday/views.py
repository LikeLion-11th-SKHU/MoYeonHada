...
def oneday_apply_create(request):
...
def oneday_recruit_create(request):
    if request.method == 'POST':
        post = OnedayRecruit()  
        post.title = request.POST['title']
        post.pub_date = timezone.now()
...
def oneday_apply_read(request):
    posts = OnedayRecruit.objects.all() 
    return render(request, 'oneday_read.html', {'posts': posts})

def oneday_recruit_read(request):
    posts = OnedayRecruit.objects.all()  
    return render(request, 'oneday_read.html', {'posts': posts})

def oneday_apply_update(request, id):
...
def oneday_recruit_update(request, id):
    update_post = get_object_or_404(OnedayRecruit, id=id)  
    update_post.title = request.POST['title']
    update_post.pub_date = timezone.now()
    update_post.body = request.POST['body']
...
def oneday_apply_delete(request, id):
    delete_post = get_object_or_404(OnedayRecruit, id=id)  
    delete_post.delete()
    return redirect('oneday_read')

def oneday_recruit_delete(request, id):
    delete_post = get_object_or_404(OnedayRecruit, id=id)  
    delete_post.delete()
    return redirect('oneday_read')